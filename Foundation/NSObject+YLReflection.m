...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] yl_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] yl_decodeType:returnType];
        NSString *encodeString = [[self class] yl_decodeType:encoding];
...
+ (NSArray *)yl_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] yl_decodeType:ivar_getTypeEncoding(ivars[i])];
...
+ (NSString *)yl_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString yl_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
